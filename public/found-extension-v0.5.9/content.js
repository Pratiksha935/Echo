(() => {
  ...
  function withoutFoundOverlay(read) {
  ...
  function ensureRoot() {
      ...
      <aside class="ec-card ec-capture-mode" aria-hidden="true">
        ...
        <div class="ec-match-view" hidden>
          ...
          <section class="ec-ask">
            <span>ASK FOUND</span>
            <form>
              <input class="found-ask-question" type="text" maxlength="700" minlength="4" placeholder="Ask what changed, who owns this, or what to do next…" required>
        ...
        <section class="ec-capture-view">
          ...
          <details class="ec-ask ec-ask-anywhere">
            <summary>Ask Found about this page</summary>
            <form>
              <input class="found-ask-question" type="text" maxlength="700" minlength="4" placeholder="Ask from company memory…" required>
              <button type="submit">ASK <b>↗</b></button>
            </form>
            <p class="ec-ask-answer" hidden></p>
          </details>
    ...
    for (const form of root.querySelectorAll(".ec-ask form")) form.addEventListener("submit", askHermes);
  ...
  async function loadSlackDestinations({ force = false } = {}) {
    ...
    slackDestinationsPromise = chrome.runtime.sendMessage({ type: "found:list-slack-destinations" })
      .then(result => {
        if (!result?.ok) {
          const error = new Error(result?.reason || "service_unavailable");
      ...
      .catch(error => {
        ...
        const message = reason === "slack_not_connected"
          ? "Connect Slack in Found to share this page with teammates or channels."
          : reason === "slack_reconnect_required" || reason === "slack_targets_unavailable"
            ? "Reconnect Slack in Found so it can list teammates and channels."
            : reason === "not_connected" || reason === "session_expired"
        ...
        note.textContent = reason === "not_connected" || reason === "session_expired"
          ? "Connect this browser to Found, then reopen this panel."
          : "Reconnect Slack from Found integrations, then reopen this panel.";
      })
  ...
  async function shareToSlack() {
    ...
    const errors = {
      ...
      slack_targets_unavailable: "Found could not list Slack teammates or channels. Reconnect Slack from Found integrations.",
  ...
  async function askHermes(event) {
    ...
    const button = form.querySelector('button[type="submit"]');
    const answer = form.closest(".ec-ask").querySelector(".ec-ask-answer");
    ...
    const result = await chrome.runtime.sendMessage({
      ...
      ask: {
        ...(latestMatch?.id ? { recordId: latestMatch.id } : {}),
        question,
        page: pageContext(),
      },
    }).catch(() => ({ ok: false, reason: "temporary_network_failure" }));
  ...
  function render(match) {
    ...
    root.querySelector(".ec-match-view .found-ask-question").value = "";
    root.querySelector(".ec-match-view .ec-ask-answer").hidden = true;
    root.querySelector(".ec-match-view .ec-ask-answer").textContent = "";
