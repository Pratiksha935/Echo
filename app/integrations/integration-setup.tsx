  ...
  return <main className={styles.page}>
    ...
    <section className={styles.steps}>
      ...
      <Step id="extension" number="04" title="Install the extension" status={extensionInstallUrl ? "STORE LISTING AVAILABLE" : "PRIVATE PREVIEW · NOT VERIFIED"}>
        {extensionInstallUrl ? <><p>Install Found from the configured Chrome Web Store listing. Found cannot verify installation from this page; confirm it in Chrome before pairing.</p><a className={styles.action} href={extensionInstallUrl} target="_blank" rel="noreferrer">OPEN CHROME WEB STORE ↗</a></> : <><p>The Chrome Web Store release is not configured for this build. For the CTO/private preview, use the approved extension folder supplied with the build.</p><ol><li>Open <code>chrome://extensions</code> in Chrome.</li><li>Turn on <b>Developer mode</b>.</li><li>Choose <b>Load unpacked</b> and select the supplied <code>extension</code> folder.</li><li>Pin Found from the Extensions menu.</li></ol><div className={styles.pending}>Production store distribution pending · no one-click install is being claimed</div></>}
      </Step>

      <Step id="pair" number="05" title="Pair this browser" status="AWAITING EXTENSION CONFIRMATION">
        <p>Open the Found extension from Chrome’s toolbar and choose <b>Connect workspace</b>. Chrome opens a secure Found confirmation using this signed-in account, then returns a short-lived, workspace-bound browser session to the extension.</p>
