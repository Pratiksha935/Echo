...
export async function POST(request: NextRequest) {
  ...
  if (!isFounderAccessEmail(email)) return failure(request, email, returnTo, "access_denied");
  try {
    const session = await exchangeOtp(email, token);
    if (!isFounderAccessEmail(session.user?.email)) return failure(request, email, returnTo, "access_denied");
