...
export default async function DecisionTimelinePage({params}:{params:Promise<{recordId:string}>}) {
  ...
  await requireFoundUser(`/workspace/decision/${encodeURIComponent(recordId)}`);
  ...
  const sourceEvents=records.filter(record=>normaliseTitle(record.title)===titleKey).map(record=>({at:record.sourceUpdatedAt,body:record.body,kind:record.source,label:record.status,url:record.sourceUrl}));
  const updateEvents=decision.updates.map(update=>({at:update.createdAt,body:update.updateText,kind:update.origin==="slack"?"Slack update":"Team update",label:"Append-only memory",url:update.sourceUrl}));
