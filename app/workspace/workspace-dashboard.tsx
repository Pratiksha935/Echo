...
export default function WorkspaceDashboard({connectedCount,demoMode,displayName,records,workspaceName}:Props) {
  ...
  const companyRecords=useMemo(()=>demoMode?demoRecords:records.map(item=>({
    team:item.department??"Company",title:item.title,owner:item.authorName??"Unknown owner",state:"Indexed",source:`${item.source} · ${item.externalId}`,keys:`${item.title} ${item.department??""}`
  })),[demoMode,records]);
  ...
  const visibleRecords=useMemo(()=>view==="Overview"?companyRecords:companyRecords.filter(item=>item.team.toLowerCase()===view.toLowerCase()),[companyRecords,view]);
  ...
  return <main className="foundWorkspace">
    ...
    <section className="wsMain">
      ...
      {!demoMode&&<section className="wsSection wsLiveMemory"><header><span>{view.toUpperCase()} MEMORY</span><h2>{records.length?"Evidence from your connected sources.":"Connect a source to begin."}</h2></header>{visibleRecords.length?<div className="wsLiveRecords">{visibleRecords.map(item=><article key={`${item.source}-${item.title}`}><span>{item.team}</span><div><h3>{item.title}</h3><p>{item.owner} · {item.source}</p></div><b>INDEXED</b></article>)}</div>:<div className="wsEmpty"><p>No company records are visible in this view. Found does not substitute demo data or another company’s knowledge.</p><Link href="/integrations">Connect Slack, Notion, Jira, Drive, or GitHub ↗</Link></div>}</section>}
