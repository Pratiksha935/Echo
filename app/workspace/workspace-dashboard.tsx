...
export default function WorkspaceDashboard({connectedCount,demoMode,displayName,records,workspaceName}:Props) {
  ...
  return <main className="foundWorkspace">
    ...
    <section className="wsMain">
      ...
      {!demoMode&&<section className="wsSection wsLiveMemory"><header><span>{view.toUpperCase()} MEMORY</span><h2>{records.length?"Decisions, evidence and outcomes—without the noise.":"Connect a source to begin."}</h2></header>{visibleRecords.length?<div className="wsLiveRecords">{visibleRecords.map(item=><article key={`${item.source}-${item.title}`}><span>{item.team}</span><div><h3><a href={item.sourceUrl} target="_blank" rel="noreferrer">{item.title}</a></h3><p>{item.owner} · {item.source}</p></div><b>{item.state}</b></article>)}</div>:<div className="wsEmpty"><p>No company records are visible in this department.</p><Link href="/integrations">Manage Slack and Google Workspace ↗</Link></div>}</section>}
