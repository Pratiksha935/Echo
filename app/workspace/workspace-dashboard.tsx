...
function DecisionRow({decision,rank}:{decision:DecisionMemory;rank?:number}) { const sourceUrl=decision.sources[0]?.url; return <article className="kbDecisionRow"><span>{rank?String(rank).padStart(2,"0"):icon(decision.department)}</span><div><div><em>{decision.department}</em><small>{decision.status}</small></div><h3>{sourceUrl?<a href={sourceUrl} target="_blank" rel="noreferrer">{decision.title} ↗</a>:<Link href={decisionPath(decision.id)}>{decision.title}</Link>}</h3><p>{truncate(decision.presentation.summary,130)}</p>{decision.presentation.kind==="spreadsheet"&&decision.presentation.fields.length>0&&<dl className="kbRecordFields">{decision.presentation.fields.slice(0,3).map(field=><div key={field.label}><dt>{field.label}</dt><dd>{field.value}</dd></div>)}</dl>}</div><aside><b>{decision.sources.length}</b><small>receipts</small><time>{formatRelative(decision.latestAt)}</time><Link href={decisionPath(decision.id)}>Memory →</Link></aside></article>; }
...
function record(externalId:string,department:string,title:string,authorName:string,status:string,source:string,sourceUrl:string,body:string,sourceUpdatedAt:string):WorkspaceKnowledgeRecord { return {externalId,department,title,authorName,status,source,sourceUrl,body,sourceUpdatedAt,metadata:{status}}; }
