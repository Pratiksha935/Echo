...
const DOMAIN_TERMS = {
  ...
  Sales: ["abm", "account", "call center", "contact center", "customer", "escalation", "handoff", "human in the loop", "pilot", "proof of value", "roi", "sales", "support", "success metric", "voice agent"],
...
const CONCEPT_GROUPS = [
  {
    label: "human handoff support workflow",
    terms: ["call center", "contact center", "customer support", "escalation", "handoff", "human in loop", "human in the loop", "human-in-the-loop", "live agent", "support agent", "voice agent"],
  },
  {
    label: "account based go to market workflow",
    terms: ["abm", "account based", "target account", "named account", "sales outreach", "pipeline", "prospect"],
  },
  {
    label: "controlled launch workflow",
    terms: ["canary", "feature flag", "progressive delivery", "rollback", "staged rollout", "deployment verification"],
  },
];
...
export function matchBrowserKnowledge({ records, pageText, pageTitle, pageUrl }) {
  ...
  if (!includeSourceAsQuery && !pageDomains.size) return null;
  const ranked = records.map(record => {
    ...
    const matchedConcepts = matchingConcepts(record, pageCorpus);
    ...
    const score = phraseScore + Math.min(8, termScore) + domainScore + conceptScore;
    return { record, terms: [...matchedPhrases, ...matchedConcepts, ...matchedTerms], matchedPhrases, matchedTerms: matchedTerms.length + matchedConcepts.length * 2, score };
  }).filter(candidate => strongCompanyEvidence(candidate.record))
  ...
  const sameDecisionRecord = sourceRecord
    ? records.find(record => strongCompanyEvidence(record) && record.externalId !== sourceRecord.externalId && normaliseTitle(record.title) === normaliseTitle(sourceRecord.title))
    : null;
  const best = ranked[0]
...
function domainCompatible(record, pageDomains, sourceRecord) {
  ...
  if (!pageDomains.size) return false;
...
function matchingConcepts(record, pageCorpus) {
  const evidence = normaliseText(`${record.title} ${record.body} ${record.department ?? ""}`);
  return CONCEPT_GROUPS
    .filter(group => group.terms.some(term => pageCorpus.includes(normaliseText(term))) && group.terms.some(term => evidence.includes(normaliseText(term))))
...
function strongCompanyEvidence(record) {
  return Boolean(record && /^(slack|google docs|google sheets|google drive|jira|notion|github|pull request|code)$/i.test(record.source));
}
