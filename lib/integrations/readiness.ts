...
export function configuredIntegrationProviders(): IntegrationProvider[] {
  const core = has("NEXT_PUBLIC_APP_URL", "NEXT_PUBLIC_SUPABASE_URL", "SUPABASE_SERVICE_ROLE_KEY", "INTEGRATION_ENCRYPTION_KEY");
  if (!core) return [];
  const ready: IntegrationProvider[] = [];
  if (has("SLACK_CLIENT_ID", "SLACK_CLIENT_SECRET")) ready.push("slack");
