...
The previous v0.4.3 content script made the authenticated match request from the visited-page execution context. That made the request-origin boundary dependent on content-script cross-origin behaviour instead of the extension service worker. From v0.4.4 onward, matching travels through a `found:match-page` runtime message; only `background.js` reads the browser token and calls the production API. The visited page never receives the token. v0.4.5 added append-only updates and resilient automatic matching. v0.4.6 added explicit department capture, decision-timeline routing and receipt deduplication. v0.4.7 reads Slack's newest rendered messages and rechecks after Slack SPA updates. v0.4.8 turns append confirmation into an authenticated decision-timeline handoff. v0.4.9 suppresses matching on Found's own production and deploy-preview pages to prevent redirect loops. v0.5.0 polishes the popup and battlecard, reruns matching after pairing, and keeps source receipts deduplicated. v0.5.1 keeps the avatar always available, suppresses auto-battlecards immediately after receipt navigation, and tightens browser matching with domain-specific phrase gates. v0.5.2 adds no-reload URL capture, a saved-memory handoff, and explicit Slack sharing to selected teammates or joined channels. v0.5.3 explains Slack reconnect failures and direct-loads exact Google source records before recent-memory fallback. v0.5.4 compacts the saved-memory state so the capture panel does not leave a blank action row. v0.5.5 treats Slack web as a message stream and matches only the newest visible human message.
...
- the downloadable ZIP contains manifest v0.5.5 and is byte-identical to all required extension source files;
